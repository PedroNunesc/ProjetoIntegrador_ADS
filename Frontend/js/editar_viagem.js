...
async function loadTrip() {
  try {
    const res = await fetch(`https://pi-back-end-oip6.onrender.com/api/trip/${tripId}`, { headers: { Authorization: "Bearer " + token } });
...
document.getElementById("btn-save").addEventListener("click", async () => {
  try {
    const res = await fetch(`https://pi-back-end-oip6.onrender.com/api/trip/${tripId}`, {
      method: "PUT",
...
document.getElementById("btn-delete").addEventListener("click", async () => {
  ...
  try {
    const res = await fetch(`https://pi-back-end-oip6.onrender.com/api/trip/${tripId}`, { method: "DELETE", headers: { Authorization: "Bearer " + token } });
